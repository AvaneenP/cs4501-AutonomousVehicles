...
def getRangeForAngle(angle, data):
    ...
    index = min(int(((angle - math.degrees(scans.angle_min)) / math.degrees(scans.angle_increment))), len(data.ranges)-1)
...
def getSteeringAngle(data, wpt):
    ...
    for i in range(-10, 11):
        dist = getRangeForAngle(i, scans) 
        if dist <= 0.5 and dist >= 0.1:
            speed = 0.0
            break 
        elif dist <= 1.0:
            speed = 15.0 
...
def path(odom_data):
    ...
    target_angle, speed = getSteeringAngle(odom_data, waypoints[goal_wpt])
